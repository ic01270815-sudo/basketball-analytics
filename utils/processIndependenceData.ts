...
export function processIndependenceData(rawData: GameRecord[]): Team {
  ...
  playerMap.forEach((games, playerName) => {
    ...
    const total2FGA = playedGames.reduce((sum, g) => sum + g['2FGA'], 0);
    const total3FGA = playedGames.reduce((sum, g) => sum + g['3FGA'], 0);
    const totalFTA = playedGames.reduce((sum, g) => sum + g.FTA, 0);
    const totalOREB = playedGames.reduce((sum, g) => sum + g.OREB, 0);
    const totalDREB = playedGames.reduce((sum, g) => sum + g.DREB, 0);
    const totalAST = playedGames.reduce((sum, g) => sum + g.AST, 0);
    const totalTO = playedGames.reduce((sum, g) => sum + g.TO, 0);
    const totalSTL = playedGames.reduce((sum, g) => sum + g.STL, 0);
    const totalBLK = playedGames.reduce((sum, g) => sum + g.BLK, 0);
  ...
  const totalTeam2FGA = rawData
    .filter(r => r.MINS > 0)
    .reduce((sum, g) => sum + g['2FGA'], 0) / totalGames;
  
  const totalTeam3FGA = rawData
    .filter(r => r.MINS > 0)
    .reduce((sum, g) => sum + g['3FGA'], 0) / totalGames;
  
  const totalTeamFTA = rawData
    .filter(r => r.MINS > 0)
    .reduce((sum, g) => sum + g.FTA, 0) / totalGames;
  
  const totalTeamOREB = rawData
    .filter(r => r.MINS > 0)
    .reduce((sum, g) => sum + g.OREB, 0) / totalGames;
  
  const totalTeamDREB = rawData
    .filter(r => r.MINS > 0)
    .reduce((sum, g) => sum + g.DREB, 0) / totalGames;
